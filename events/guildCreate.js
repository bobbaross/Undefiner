...
module.exports = (client, guild) => {
    async function isBanned() {
        client.functions.getStaffDB().then(res => {
            ...
            if (res?.bannedServers.find(item => item.guild === guild.id)) {
                let item = res.bannedServers.find(item => item.guild === guild.id);
                try {
            ...
            else if (res?.bannedOwners.find(item => item.owner === guild.ownerID)) {
                let item = res.bannedOwners.find(item => item.owner === guild.ownerID);
                try {
                    if (sysChan.permissionOverwrites.get(client.user.id).allow.has("SEND_MESSAGES")) {
