...
module.exports = {
    name: "addmessages",
    description: "Add messages for members during competitions.",
    usage: "<member> <amount>",
    aliases: [],
    ...
    async undefine(client, message, args, hasEmbedPerms) {
        client.functions.getDB(message.guild.id).then(async res => {
            ...
            if (!amount || isNaN(amount)) {
                let embed = new MessageEmbed()
                .setColor(branding)
                .setDescription(`Please tell me the amount to add.`)
                if (hasEmbedPerms === true) {
                    return message.channel.send(embed).catch(err => err);
                } else {
                    return message.channel.send(embed.description).catch(err => err);
            ...
            let embed = new MessageEmbed()
            .setColor(branding)
            .setDescription(`Successfully added ${amount} messages to ${user.tag}`)
            if (hasEmbedPerms === true) {
                return message.channel.send(embed).catch(err => err);
            } else {
                return message.channel.send(embed.description).catch(err => err);
