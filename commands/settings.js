            ...
            switch (args[0].toLowerCase()) {
                ...
                case "mutedrole":
                    ...
                    return message.channel.send(embed.description).catch(err => err)
                        ...
                        let channelTwo;
                        if (channel) {channel = channel.id; channelTwo = await getChannel(args[1], message.guild.channels.cache)}
                        else if (args[1].toLowerCase() === "this") {channel = message.channel.id; channelTwo = await getChannel(message.channel.id, message.guild.channels.cache)}
