...
module.exports = {
    name: "funban",
    description: "(FAKE) Unban a member.",
    aliases: ['funbanish', "fub"],
    category: "fun",
    usage: "<member> <reason>",
    guildOnly: true,
    ...
    async undefine(client, message, args, hasEmbedPerms) {
        client.functions.getModDB(message.guild.id).then(async res => {
            ...
            client.functions.getSettingsDB(message.guild.id).then(async setRes => {
                ...
                var embedId;
                var modLogsChan = message.channel;//await client.functions.getChannel(setRes.settings.modLogs, message.guild.channels.cache);
                /*if (modLogsChan || setRes.settings.modLogs === "there") {
                    ...
                    embedId = await new Promise(resolve => {
                        ...
                        .setColor(good)
                        .setTitle(`Member Unbanned | Case #${Math.round(Math.random()*(message.guild.memberCount*5 - message.guild.memberCount))}`)
